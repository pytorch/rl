...
parser.add_argument("--gpus_per_node", "--gpus-per-node", "-G", type=int, default=0, help="Number of GPUs per node. If greater than 0, the backend used will be NCCL.")
parser.add_argument("--cpus_per_task", "--cpus-per-task", "-c", type=int, default=32, help="Number of CPUs per node.")
...
@submitit_delayed_launcher(num_jobs=num_jobs, backend="nccl" if slurm_gpus_per_node else "gloo", tcpport=tcp_port)
def main():
