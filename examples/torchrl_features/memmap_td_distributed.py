...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        if args.task == 0:
            ...
            time.sleep(1)
            t0 = time.time()
            waiters = [rpc.remote(f"worker{w}", get_tensordict, args=tuple()) for w in range(1, args.world_size)]
            td = torch.stack([waiter.to_here() for waiter in waiters], 0).contiguous()
            print("time: ", time.time() - t0)

            t0 = time.time()
            waiters = [rpc.remote(f"worker{w}", tensordict_add, args=tuple()) for w in range(1, args.world_size)]
            td = torch.stack([waiter.to_here() for waiter in waiters], 0).contiguous()
            print("time: ", time.time() - t0)
            ...
            t0 = time.time()
            if args.memmap:
                waiters = [rpc.remote(f"worker{w}", tensordict_add_noreturn, args=tuple()) for w in range(1, args.world_size)]
                print('temp t: ', time.time() - t0)
                [waiter.to_here() for waiter in waiters] # the previous stack will track the original files
                print('temp t: ', time.time() - t0)
            else:
                waiters = [rpc.remote(f"worker{w}", tensordict_add, args=tuple()) for w in range(1, args.world_size)]
                print('temp t: ', time.time() - t0)
