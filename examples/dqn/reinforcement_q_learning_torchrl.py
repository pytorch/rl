# -*- coding: utf-8 -*-
"""
This is a PyTorch RL package adaptation of the PyTorch tutorial 
...
if len (info) != 0 :
    print("info: ", info)
n_observations = state["observation"].numel()
...
def optimize_model():
    ...
    #non_final_mask = torch.tensor(tuple(map(lambda s: s is not torch.equal(s, finalmark),
    #                                      batch.next_state)), device=device, dtype=torch.bool)
    #non_final_next_states = torch.cat([s for s in batch.next_state
    #                                            if s is not finalmark])
    #state_batch = torch.cat(batch.state)
    #action_batch = torch.cat(batch.action)
    #reward_batch = torch.cat(batch.reward)

    nfidx = batch.next_state != FINALMARK
    non_final_mask = torch.sum(nfidx, 1) != 0
    ...
    state_action_values_all = policy_net(state_batch)
    #state_action_values = state_action_values_all.gather(1, action_batch.squeeze((1,2)))
    state_action_values = state_action_values_all.gather(1, action_batch.argmax(1).unsqueeze(1))# categorical to one-hot
...
FINALMARK = torch.tensor([-999999.0, -999999.0,-999999.0,-999999.0]).to(device)
...
for _i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        tdnext = env.step(tdact)
        observation = tdnext["next"]["observation"]
        reward = tdnext["next"]["reward"]
        terminated = tdnext["next"]["terminated"] 
        truncated  = tdnext["next"]["truncated"]
        done = terminated or truncated

        if terminated:
            next_state = FINALMARK
        else:
