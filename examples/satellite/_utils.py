...
* :func:`eval_metrics_fn` -- per-category metrics for the
  :class:`Evaluator`.
* :func:`setup_wandb` -- exports the user's *personal* WandB key from
  ``~/.tokens`` before importing wandb.

Run ``python -m examples.satellite._utils generate-test-set`` to dump the CSV.
"""
...
def pick_device(prefer: str | None = None) -> torch.device:
    ...
    .. note::
        ...
        The user said "GPU if available, CPU otherwise" -- on Apple
        silicon that means CPU.
    """
...
def make_eval_env(
    *,
    ...
    rewards are directly comparable.
    """
...
def setup_wandb_key(tokens_path: str | Path = "~/.tokens") -> None:
    """Export the user's *personal* WandB key from ``~/.tokens``.

    The shared file mixes a Periodic key and a personal key on adjacent
    lines; we look specifically for ``Personal: WANDB_API_KEY=...``.
    Sets ``WANDB_API_KEY`` in ``os.environ``; raises if the personal key
    is not found (per the user's hard rule: never the Periodic key).

    If ``WANDB_API_KEY`` is already set in the environment (e.g. a remote
    run where ``~/.tokens`` is not present), this is a no-op.
    """
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"No tokens file at {path}; cannot find personal WANDB key."
        )
    personal_key: str | None = None
    for line in path.read_text().splitlines():
        s = line.strip()
        if s.startswith("Personal:") and "WANDB_API_KEY=" in s:
            personal_key = s.split("WANDB_API_KEY=", 1)[1].strip()
            break
    if not personal_key:
        raise RuntimeError(
            "Could not find a 'Personal: WANDB_API_KEY=...' line in "
            f"{path}. Refusing to fall back to a non-personal key."
        )
    os.environ["WANDB_API_KEY"] = personal_key
