...
def init_transformer(config, as_tensordictmodule=True, skip_compilation=False, inference=False):
    model_kwargs = {
    ...
    if config["init_base_from"] in ["scratch", "pretrained"]:
        model = GPT2.from_pretrained(config["base_model"], return_dict=False, **model_kwargs)
        if config["init_base_from"] == "scratch":
            model.post_init()
    elif config["init_base_from"] == "resume":
        model = GPT2.from_pretrained(config["out_dir"], return_dict=False)
    else:
