        ...
        if (step % cfg.train.logging_frequency) == 0:
            with torch.no_grad():
                metrics = {
                    "reward": float(
                        torch.cat(
                            replay_buffer[:].get(("next", "reward"), as_list=True)
                        ).mean()
                    ),
                    "kl_penalty": float(
                        torch.cat(
                            replay_buffer[:].get(("next", "kl_penalty"), as_list=True)
                        ).mean()
                    ),
                    "seq_length": float(
                        torch.tensor(
                    ...
                    "gradient_step_throughput": float(step / replay_buffer.write_count),
                    # how many optim steps per write
                    "optim_step_throughput": float(
                    ...
                    "batch_policy_version": batch["next", "policy_version"]
                    .view(-1).min()
                }
