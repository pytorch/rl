...
def make_offline_replay_buffer(rb_cfg, cfg, device):
    ...
    Otherwise, collects random rollouts from the environment (useful
    for CI testing but not for meaningful training).
    """
    if rb_cfg.dataset:
    ...
    # Collect random rollouts (no expert — just for pipeline testing)
