...
if [[ "$TORCH_VERSION" == "nightly" ]]; then
  if [ "${CU_VERSION:-}" == cpu ] ; then
      pip3 install mpmath==1.3 --pre torch torchvision --index-url https://download.pytorch.org/whl/nightly/cpu -U
  else
      pip3 install mpmath==1.3 --pre torch torchvision --index-url https://download.pytorch.org/whl/nightly/$CU_VERSION  -U
  fi
