...
run_distributed_tests() {
  ...
  # Run both test/test_distributed.py and test/rb/test_distributed.py (both use torch.distributed)
  # Note: distributed tests always run on GPU, no need for GPU_MARKER_FILTER here
  python .github/unittest/helpers/coverage_run_parallel.py -m pytest test/test_distributed.py test/rb/test_distributed.py \
    ${json_report_args} \
...
run_non_distributed_tests() {
  ...
  local common_ignores="--ignore test/test_rlhf.py --ignore test/test_distributed.py --ignore test/rb/test_distributed.py --ignore test/llm --ignore test/test_setup.py"
