...
if [[ "$TORCH_VERSION" == "nightly" ]]; then
  if $torch_cuda ; then
    python -m pip install mpmath==1.3 --pre torch --index-url https://download.pytorch.org/whl/nightly/cu118
  else
    python -m pip install mpmath==1.3 --pre torch --index-url https://download.pytorch.org/whl/nightly/cpu -U
  fi
