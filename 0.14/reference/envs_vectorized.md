...
Some libraries such as [gym3](https://github.com/openai/gym3) or [EnvPool](https://github.com/sail-sg/envpool)
...
Therefore, TorchRL offers its own, generic [`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv) class to run multiple
environments in parallel.
As this class inherits from [`SerialEnv`](generated/torchrl.envs.SerialEnv.html#torchrl.envs.SerialEnv), it enjoys the exact same API as other environment.
Of course, a [`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv) will have a batch size that corresponds to its environment count:
...
[`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv) will create buffers from these specs to communicate with the spawn processes.
Check the [`check_env_specs()`](generated/torchrl.envs.check_env_specs.html#torchrl.envs.check_env_specs) method for a sanity check.
...
[`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv) allows to retrieve the attributes from its contained environments:
one can simply call:
...
*A note on performance*: launching a [`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv) can take quite some time
...
that [`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv) (as well as data collectors)
...
In general, an environment should be tested using the [`check_env_specs()`](generated/torchrl.envs.check_env_specs.html#torchrl.envs.check_env_specs)
test function before being used in a [`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv). This function will raise
an assertion error whenever the preallocated buffer and the collected data mismatch.

We also offer the [`SerialEnv`](generated/torchrl.envs.SerialEnv.html#torchrl.envs.SerialEnv) class that enjoys the exact same API but is executed
serially. This is mostly useful for testing purposes, when one wants to assess the
behavior of a [`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv) without launching the subprocesses.

In addition to [`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv), which offers process-based parallelism, we also provide a way to create
multithreaded environments with `MultiThreadedEnv`. This class uses [EnvPool](https://github.com/sail-sg/envpool)
...
etc.), but one can not use an arbitrary TorchRL environment, as it is possible with [`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv). Run
...
| [`SerialEnv`](generated/torchrl.envs.SerialEnv.html#torchrl.envs.SerialEnv)(*args, **kwargs) | Creates a series of environments in the same process.Batched environments allow the user to query an arbitrary method / attribute of the environment running remotely. |
| --- | --- |
| [`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv)(*args, **kwargs) | Creates one environment per process. |
| [`EnvCreator`](generated/torchrl.envs.EnvCreator.html#torchrl.envs.EnvCreator)(create_env_fn[, ...]) | Environment creator class. |
...
This is controlled via the `batch_locked` attribute: a batch-locked environment requires all input
tensordicts to have the same batch-size as the env's. Typical examples of these environments are
[`GymEnv`](generated/torchrl.envs.GymEnv.html#torchrl.envs.GymEnv) and related. Batch-unlocked envs are by contrast allowed to work with any input size.
Notable examples are [`BraxEnv`](generated/torchrl.envs.BraxEnv.html#torchrl.envs.BraxEnv) or [`JumanjiEnv`](generated/torchrl.envs.JumanjiEnv.html#torchrl.envs.JumanjiEnv).
...
Batched environments ([`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv) and [`SerialEnv`](generated/torchrl.envs.SerialEnv.html#torchrl.envs.SerialEnv)) can also deal with
...
This means that custom environments may silently run the non-required steps as there is no way for torchrl
...
- Batched environments: [`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv) and [`SerialEnv`](generated/torchrl.envs.SerialEnv.html#torchrl.envs.SerialEnv) will dispatch the
action to and only to the environments where "_step" is True;
- Batch-unlocked environments;
- Unbatched environments (i.e., environments without batch size). In these environments, the [`step()`](generated/torchrl.envs.EnvBase.html#id4)
method will first look for a "_step" entry and, if present, act accordingly.
If a [`Transform`](generated/torchrl.envs.transforms.Transform.html#torchrl.envs.transforms.Transform) instance passes a "_step" entry to the tensordict, it is also captured by
[`TransformedEnv`](generated/torchrl.envs.transforms.TransformedEnv.html#torchrl.envs.transforms.TransformedEnv)'s own _step method which will skip the base_env.step as well as any further
transformation.
...
Partial steps are an essential feature of [`rollout()`](generated/torchrl.envs.EnvBase.html#id2) when break_when_all_done is True,
as the environments with a True done state will need to be skipped during calls to _step.

The [`ConditionalSkip`](generated/torchrl.envs.transforms.ConditionalSkip.html#torchrl.envs.transforms.ConditionalSkip) transform allows you to programmatically ask for (partial) step skips.
...
Likewise, partial resets are an essential feature of [`rollout()`](generated/torchrl.envs.EnvBase.html#id2) when break_when_any_done is True,
...
1. During a call to `_reset()`, the `"_reset"` key may or may
...
The way an environment deals with the `"_reset"` keys in its `_reset()`
...
of `_reset()`. Nevertheless, the following point should be
kept in mind when designing that method.
2. After a call to `_reset()`, the output will be masked with the
`"_reset"` entries and the output of the previous [`step()`](generated/torchrl.envs.EnvBase.html#id4)
...
entries will be erased from the [`reset()`](generated/torchrl.envs.EnvBase.html#id1) outputs.
...
`PettingZooWrapper` implementation where each group has one or more
`"done"` entries associated which is aggregated at the root level with a
`any` or `all` logic depending on the task).
- When calling `env.reset(tensordict)()` with a partial `"_reset"` entry
...
Contrary to [`ParallelEnv`](generated/torchrl.envs.ParallelEnv.html#torchrl.envs.ParallelEnv), [`AsyncEnvPool`](generated/torchrl.envs.AsyncEnvPool.html#torchrl.envs.AsyncEnvPool) and its subclasses permit the
...
| [`AsyncEnvPool`](generated/torchrl.envs.AsyncEnvPool.html#torchrl.envs.AsyncEnvPool)(*args, **kwargs) | A base class for asynchronous environment pools, providing a common interface for managing multiple environments concurrently. |
| --- | --- |
| [`ProcessorAsyncEnvPool`](generated/torchrl.envs.ProcessorAsyncEnvPool.html#torchrl.envs.ProcessorAsyncEnvPool)(*args, **kwargs) | An implementation of AsyncEnvPool using multiprocessing for parallel execution of environments. |
| [`ThreadingAsyncEnvPool`](generated/torchrl.envs.ThreadingAsyncEnvPool.html#torchrl.envs.ThreadingAsyncEnvPool)(*args, **kwargs) | An implementation of AsyncEnvPool using threading for parallel execution of environments. |