...
*class*torchrl.envs.SatelliteEnv(**args*, *num_workers: int = 1*, *parallel: bool | None = None*, ***kwargs*)[[source]](../../_modules/torchrl/envs/custom/mujoco/satellite.html#SatelliteEnv)
...
`_prepare_ctrl()`: the agent emits actions in
...
Defaults to `ROTOR_SPEED` (proxy unit, since the
...
```
>>> from torchrl.envs import SatelliteEnv 
>>> env = SatelliteEnv(num_cmgs=4, num_envs=4) 
>>> td = env.rollout(50)
```
...
RENDER_BACKGROUND*: ClassVar[tuple[float, float, float] | None]**= (0.0, 0.0, 0.05)*
...
make_attitude_transform(***, *execute: bool = True*, *multi_action_dim: int = 1*, *stack_rewards: bool = True*, *stack_observations: bool = False*, *macro_steps: int = 36*, *settle_steps: int = 8*, *attitude_gain: float = 5.0*, *angular_rate_gain: float = 8.0*, *jacobian_rotor_h: float = 1.0*) → [Transform](torchrl.envs.transforms.Transform.html#torchrl.envs.transforms.Transform)[[source]](../../_modules/torchrl/envs/custom/mujoco/satellite.html#SatelliteEnv.make_attitude_transform)
...
```
>>> from torchrl.envs import SatelliteEnv 
>>> env = SatelliteEnv(num_cmgs=4) 
>>> transform = env.make_attitude_transform(execute=True)
```
