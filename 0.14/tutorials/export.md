...
class GraphModule(torch.nn.Module):
 ...
 # File: /pytorch/rl/torchrl/envs/transforms/_base.py:572 in forward, code: data = self._apply_transform(data)
 ...
 # File: /pytorch/rl/torchrl/modules/tensordict_module/actors.py:864 in forward, code: action = self.action_func_mapping[self.action_space](action_values)
 ...
 # File: /pytorch/rl/torchrl/modules/tensordict_module/actors.py:869 in forward, code: chosen_action_value = action_value_func(action_values, action)
...
class GraphModule(torch.nn.Module):
 ...
 # File: /opt/conda/lib/python3.11/site-packages/tensordict/nn/sequence.py:651 in forward, code: tensordict_exec = self._run_module(
 ...
 # File: /pytorch/rl/torchrl/modules/tensordict_module/rnn.py:1357 in forward, code: tensordict_shaped = tensordict.reshape(-1).unsqueeze(-1)
 ...
 # File: /pytorch/rl/torchrl/modules/tensordict_module/rnn.py:1359 in forward, code: is_init = tensordict_shaped["is_init"].squeeze(-1)
 squeeze: "b8[1]" = torch.ops.aten.squeeze.dim(unsqueeze_7, -1)

 # File: /pytorch/rl/torchrl/modules/tensordict_module/rnn.py:1397 in forward, code: is_init_expand = expand_as_right(is_init, hidden0)
 ...
 # File: /pytorch/rl/torchrl/modules/tensordict_module/rnn.py:1398 in forward, code: zeros = torch.zeros_like(hidden0)
 zeros_like: "f32[1, 1, 2, 256]" = torch.ops.aten.zeros_like.default(unsqueeze_5, pin_memory = False)

 # File: /pytorch/rl/torchrl/modules/tensordict_module/rnn.py:1399 in forward, code: hidden0 = torch.where(is_init_expand, zeros, hidden0)
 where: "f32[1, 1, 2, 256]" = torch.ops.aten.where.self(expand, zeros_like, unsqueeze_5); unsqueeze_5 = None

 # File: /pytorch/rl/torchrl/modules/tensordict_module/rnn.py:1400 in forward, code: hidden1 = torch.where(is_init_expand, zeros, hidden1)
 where_1: "f32[1, 1, 2, 256]" = torch.ops.aten.where.self(expand, zeros_like, unsqueeze_6); expand = zeros_like = unsqueeze_6 = None

 # File: /pytorch/rl/torchrl/modules/tensordict_module/rnn.py:1406 in forward, code: val, hidden0, hidden1 = self._lstm(
 ...
 # File: /pytorch/rl/torchrl/modules/tensordict_module/rnn.py:798 in forward, code: return self._lstm(input, hx, mask)
 ...
 # File: /pytorch/rl/torchrl/modules/tensordict_module/rnn.py:1427 in forward, code: tensordict.update(tensordict_shaped.reshape(shape))
...
```
pkg_path /tmp/tmp411z0e3r/model.pt2
tensor(1)
```
...
[torch.onnx] Optimize the ONNX graph... ✅
ONNX rollout took 635.4818 msec (total = 0.6355 sec since last reset).
TorchRL version took 1967.6938 msec (total = 1.9677 sec since last reset).
```
...
**Total running time of the script:** (0 minutes 23.345 seconds)
