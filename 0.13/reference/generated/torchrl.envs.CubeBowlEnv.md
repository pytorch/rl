...
*class*torchrl.envs.CubeBowlEnv(**args*, *num_workers: int = 1*, *parallel: bool | None = None*, ***kwargs*)[[source]](../../_modules/torchrl/envs/custom/mujoco/cube_bowl.html#CubeBowlEnv)

UR-style cube-to-bowl manipulation scene.
...
- ****kwargs** - forwarded to [`MujocoEnv`](torchrl.envs.MujocoEnv.html#torchrl.envs.MujocoEnv).

Examples

```
>>> from torchrl.envs import CubeBowlEnv 
>>> env = CubeBowlEnv(max_episode_steps=50) 
>>> td = env.rollout(3)
```
...
*property*gripper_close_ctrl*: float*

Low-level command that closes the gripper for object grasping.

gripper_ctrl_for_width(*width: float | [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)*) → float | [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)[[source]](../../_modules/torchrl/envs/custom/mujoco/cube_bowl.html#CubeBowlEnv.gripper_ctrl_for_width)
...
>>> env = CubeBowlEnv() 
>>> td = env.reset() 
>>> env.gripper_cube_distance(td).shape 
torch.Size([1, 1])
```

*property*gripper_open_ctrl*: float*
...
>>> env = CubeBowlEnv() 
>>> td = env.reset() 
>>> env.low_level_action(td["robot_qpos"]).shape 
torch.Size([1, 7])
```
...
make_urscript_transform(***, *macro_steps: int = 16*, *settle_steps: int = 0*, *execute: bool = True*, *multi_action_dim: int = 1*, *stack_rewards: bool = True*, *stack_observations: bool = False*, *action_key: NestedKey = 'action'*, *robot_qpos_key: NestedKey = 'robot_qpos'*, *gripper_qpos_key: NestedKey = 'gripper_qpos'*, *ik_kwargs: dict[str, float | int] | None = None*) → [Transform](torchrl.envs.transforms.Transform.html#torchrl.envs.transforms.Transform)[[source]](../../_modules/torchrl/envs/custom/mujoco/cube_bowl.html#CubeBowlEnv.make_urscript_transform)
...
passed directly to [`step()`](torchrl.envs.ModelBasedEnvBase.html#torchrl.envs.step). Pass `False` to inspect or
...
>>> env = CubeBowlEnv() 
>>> env = env.append_transform(env.make_urscript_transform(macro_steps=4)) 
>>> td = env.reset() 
>>> offset = td["cube_pos"].new_tensor([[0.0, 0.0, 0.1]]) 
>>> td["action"] = RobotMacroAction.reach_pose( 
... position=td["cube_pos"] + offset,
... quaternion=td["pinch_quat"],
... gripper="open",
... )
>>> env.step(td).get(("next", "reward")).shape 
torch.Size([1, 1])
```
...
*static*pose_at(*xyz: [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)*, *quat: [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor) | None = None*) → [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)[[source]](../../_modules/torchrl/envs/custom/mujoco/cube_bowl.html#CubeBowlEnv.pose_at)
...
render(***, *width: int | None = None*, *height: int | None = None*, *camera_id: int | None = None*) → [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)
...
*property*robot_home_qpos*: tuple[float, ...] | None*
