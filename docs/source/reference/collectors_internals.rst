...
A single iteration of :meth:`Collector.rollout` corresponds to one
...
Implementation: :meth:`Collector.rollout` in
...
:meth:`Collector._make_carrier` and then advanced at the end of every
...
:attr:`Collector.post_collect_hook` or read the data yielded by
...
:meth:`Collector._setup_devices` and called inside the rollout loop
...
``_cast_to_policy_device``
    Set in :meth:`Collector._setup_devices`.  ``True`` iff
    ``policy_device != env_device``.  When ``True``, the carrier is copied to
    ``policy_device`` before the policy is invoked.

``_cast_to_env_device``
    Set in :meth:`Collector._apply_env_device`, after the environment
...
:meth:`Collector._make_carrier`) records whether the carrier was
...
- :meth:`Collector._traj_pool` returns a process-local
...
- :meth:`Collector._update_traj_ids` runs after each env step. It reads
...
.. list-table::
   ...
   * - Per-step rollout loop
     - :meth:`Collector.rollout` in ``torchrl/collectors/_single.py``
   * - Carrier initialization
     - :meth:`Collector._make_carrier`
   * - Device setup and policy cast flag
     - :meth:`Collector._setup_devices`
   * - Environment device application and env cast flag
     - :meth:`Collector._apply_env_device`
   * - Sync function dispatch
     - :meth:`Collector._get_sync_fn`
   * - Trajectory ID update
     - :meth:`Collector._update_traj_ids`
   * - Trajectory ID pool
