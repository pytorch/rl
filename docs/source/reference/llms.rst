...
The :class:`~torchrl.data.llm.History` class is the core data structure for managing conversation context. It supports assistant token masking across 
multiple model families, allowing you to identify which tokens in a conversation were generated by the assistant - crucial for reinforcement learning 
applications.
...
.. note::
   For most RL and SFT workflows, you will interact with these output classes either in padded (tensor) or sparse (list) form, depending on your batching and memory requirements.

LLM Output Classes
^^^^^^^^^^^^^^^^^^

TorchRL provides structured output classes for handling LLM responses:
