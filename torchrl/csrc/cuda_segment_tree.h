...
class CudaSegmentTree {
 public:
  ...
  T At(int64_t index) const {
    return values_.index({index | capacity_}).item<T>();
  }
  ...
  py::array_t<T> At(const py::array_t<int64_t>& index) const {
    ...
    std::memcpy(value.mutable_data(), cpu_value.data_ptr<T>(),
                index.size() * sizeof(T));
    return value;
  }
  ...
  T Query(int64_t l, int64_t r) const {
    ...
    auto r_tensor = torch::tensor(
        {r},
        torch::TensorOptions().dtype(torch::kInt64).device(values_.device()));
    return Query(l_tensor, r_tensor).cpu().item<T>();
  }
  ...
  py::array_t<T> Query(const py::array_t<int64_t>& l,
    ...
    std::memcpy(ret.mutable_data(), cpu_result.data_ptr<T>(),
                l.size() * sizeof(T));
    return ret;
  }
  ...
  py::array_t<T> DumpValues() const {
    ...
    std::memcpy(ret.mutable_data(), leaves.data_ptr<T>(), size_ * sizeof(T));
    return ret;
  }
...
class CudaSumSegmentTree final
 ...
 public:
  ...
  int64_t ScanLowerBound(const T& value) const {
    ...
    return ScanLowerBound(value_tensor).cpu().item<int64_t>();
  }
  ...
  py::array_t<int64_t> ScanLowerBound(const py::array_t<T>& value) const {
    ...
    std::memcpy(index.mutable_data(), cpu_index.data_ptr<int64_t>(),
