...
from torchrl.data.vla.schema import ACTION_CHUNK_KEY, ACTION_KEY
...
class ActionChunkExecutor(TensorDictModuleBase):
    """Execute a chunk-predicting policy one action per environment step.

    Chunk-predicting policies (action-chunking VLAs, ACT, diffusion policies)
    predict an *action chunk* (``H`` future actions) per inference, but
    environments consume one action per step. This wrapper turns a
    chunk-predicting policy into a step policy: it caches the predicted chunk and
    emits one action per call, only re-invoking the (expensive) policy every
    ``replan_interval`` steps (receding horizon; ``replan_interval == chunk_size``
    is fully open-loop). It tracks the per-environment position in the current
    chunk and re-plans an environment whenever it is reset (``is_init``), so it
    behaves correctly inside a batched collector. Use it as the policy of a
    collector or :meth:`~torchrl.envs.EnvBase.rollout`.

    Args:
        policy (TensorDictModuleBase): a policy writing an action chunk of shape
            ``[*B, chunk_size, action_dim]`` under ``action_chunk_key`` (e.g. a
            continuous-head :class:`~torchrl.modules.vla.VLAWrapperBase`).

    Keyword Args:
        replan_interval (int, optional): re-plan every ``replan_interval`` steps.
            Must be in ``[1, chunk_size]``. Defaults to ``chunk_size`` (open-loop).
        chunk_size (int, optional): the chunk horizon. Defaults to the policy's
            ``chunk_size`` attribute when available.
        action_chunk_key (NestedKey): key the policy writes the chunk to.
            Defaults to ``"action_chunk"``.
        action_key (NestedKey): key the per-step action is written to.
            Defaults to ``"action"``.
        is_init_key (NestedKey): key flagging an environment reset (re-plan).
            Defaults to ``"is_init"``.

    Examples:
        >>> import torch
        >>> from tensordict import NonTensorStack, TensorDict
        >>> from torchrl.modules import ActionChunkExecutor, TinyVLA
        >>> policy = TinyVLA(action_dim=7, chunk_size=4)
        >>> executor = ActionChunkExecutor(policy, replan_interval=4)
        >>> def obs(batch=2):
        ...     return TensorDict(
        ...         {
        ...             "observation": {
        ...                 "image": torch.zeros(batch, 3, 16, 16, dtype=torch.uint8),
        ...                 "state": torch.zeros(batch, 5),
        ...             },
        ...             "language_instruction": NonTensorStack("a", "b"),
        ...         },
        ...         batch_size=[batch],
        ...     )
        >>> executor(obs())["action"].shape
        torch.Size([2, 7])

    .. note::
        Per-environment re-planning on reset requires an ``is_init`` flag in the
        input; ``is_init`` is declared in ``in_keys`` so a collector auto-adds an
        :class:`~torchrl.envs.transforms.InitTracker`. Without it (e.g. a bare
        ``env.rollout`` with no ``InitTracker``) a freshly-reset environment
        keeps serving its cached chunk until the next scheduled re-plan.

    .. note::
        The executor holds per-environment state across calls and nothing resets
        it for you: call :meth:`reset` between independent rollouts, and keep the
        batch size constant between resets.

    .. seealso::
        :class:`~torchrl.modules.tensordict_module.MultiStepActorWrapper` solves
        the same one-action-per-step problem for open-loop chunk execution;
    ...
    def __init__(
        ...
        *,
        ...
        cs = (
            ...
            else getattr(policy, "chunk_size", None)
        )
        if cs is None:
            raise ValueError(
                "chunk_size must be provided (or the policy must expose a "
                "`chunk_size` attribute)."
            )
        self.chunk_size = int(cs)
        ri = self.chunk_size if replan_interval is None else int(replan_interval)
        if not 1 <= ri <= self.chunk_size:
            raise ValueError(
                f"replan_interval must be in [1, chunk_size={self.chunk_size}], got {ri}."
            )
        self.replan_interval = ri
        self.policy = policy
        self.action_chunk_key = action_chunk_key
        self.action_key = action_key
        self.is_init_key = is_init_key
        # declare the is_init dependency (like LSTMModule) so a collector auto-adds
        # an InitTracker and per-environment re-planning on reset works.
        in_keys = list(policy.in_keys)
        if is_init_key not in in_keys:
            in_keys.append(is_init_key)
        self.in_keys = in_keys
    ...
    def reset(self) -> None:
        """Clear the cached chunk and per-environment step counters."""
        self._chunk = None
        self._t = None

    def forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        is_init = tensordict.get(self.is_init_key, default=None)
        if self._chunk is None:
            # cold start: plan for every environment.
            self._chunk = self.policy(tensordict.copy()).get(self.action_chunk_key)
            self._t = torch.zeros(
                self._chunk.shape[:-2], dtype=torch.long, device=self._chunk.device
            )
        else:
            need_replan = self._t >= self.replan_interval
            if is_init is not None:
                need_replan = need_replan | is_init.reshape(self._t.shape).bool()
            if bool(need_replan.any()):
                # re-plan the whole batch but only commit the new chunk for the
                # environments that need it (the policy is skipped otherwise).
                chunk = self.policy(tensordict.copy()).get(self.action_chunk_key)
                self._chunk = torch.where(
                    need_replan.unsqueeze(-1).unsqueeze(-1), chunk, self._chunk
                )
                self._t = torch.where(need_replan, torch.zeros_like(self._t), self._t)

        # gather one action per environment at its current chunk position
        index = (
            self._t.unsqueeze(-1)
            .unsqueeze(-1)
            .expand(*self._t.shape, 1, self._chunk.shape[-1])
        )
        action = self._chunk.gather(-2, index).squeeze(-2)
        self._t = self._t + 1
        tensordict.set(self.action_key, action)
        return tensordict
