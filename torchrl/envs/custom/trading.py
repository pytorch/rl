...
class FinancialMarketDynamics:
    ...
    def rolling_window(self, tensor: torch.Tensor, window_size: int, dim: int = -1) -> torch.Tensor:
        """
        ...
        if window_size > seq_len:
            raise ValueError(f"Window size {window_size} cannot be larger than sequence length {seq_len}")
    ...
    def calculate_returns(self, prices: torch.Tensor, periods: int = 1) -> torch.Tensor:
        ...
        returns[..., periods:] = (prices[..., periods:] - prices[..., :-periods]) / prices[..., :-periods]
    ...
    def detect_market_regime(self, prices: torch.Tensor, lookback_days: int = 30) -> torch.Tensor:
        """
        ...
        if prices.shape[-1] < lookback_days:
            ...
            return torch.full(batch_shape, MarketRegime.SIDEWAYS, dtype=torch.long, device=self.device)
        ...
        sma_50 = self.rolling_mean(recent_prices, 50)[..., -1] if recent_prices.shape[-1] >= 50 else sma_20
        ...
        batch_shape = prices.shape[:-1]
        regime = torch.full(batch_shape, MarketRegime.SIDEWAYS, dtype=torch.long, device=self.device)
        ...
        regime = torch.where(
            ~(strong_bull_cond1 | strong_bull_cond2) & (bull_cond1 | bull_cond2 | bull_cond3),
            MarketRegime.BULL,
            regime
        )
    ...
    def calculate_price_volatility(self, prices: torch.Tensor, lookback_days: int = 30) -> torch.Tensor:
        """
...
class FinancialRegimeEnv(EnvBase):
    ...
    def _reset_random_data(self, shape, batch_size, params):
        ...
        returns = torch.randn(
            ...
            generator=self.rng, 
            device=self.device
        ) * volatility.unsqueeze(-1) * math.sqrt(dt)
...
def example_usage():
    """Example usage of the FinancialRegimeEnv."""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # Create environment
    env = FinancialRegimeEnv(device=device, window_size=50)
    
    print(f"Environment created on device: {env.device}")
    ...
    td = env.reset()
    print(f"\nInitial state:")
    print(f"Price history shape: {td['price_history'].shape}")
    print(f"Current holdings: {td['current_holdings']}")
    print(f"Initial price: {td['price_history'][..., -1]:.2f}")
    
    # Take a few steps
    for step in range(5):
        # Random action
        action = torch.randint(0, 3, (1,), device=device)
        td["action"] = action
        ...
        print(f"\nStep {step + 1}:")
        print(f"Action: {action.item()} ({'Hold' if action == 0 else 'Buy' if action == 1 else 'Sell'})")
        print(f"New price: {td['price_history'][..., -1]:.2f}")
        print(f"Holdings: {td['current_holdings'].item()}")
        print(f"Reward: {td['reward'].item():.4f}")
