...
class ChatEnv(EnvBase):
    ...
    - Given a prompt (key `("text", "prompt")`) and an answer string (key `("text", "response")`, which is our action), the environment
    ...
    - Optionally, the history is mapped back to a `("text", "prompt")` entry that can be used to query the LLM in the next round
      of the policy.
    ...
    Args:
        ...
        apply_template (bool, optional): if `True` (and a tokenizer is passed), the history will be parsed to a string
            in the `("text", "prompt")` entry of the output tensordict at reset time. Defaults to `False`.
            ...
            .. note:: If transforms are appended to the environment, the template will be applied to the history before the transform is applied.
                ...
                will apply the template to the history when queried if no `("text", "prompt")` input is provided.
    ...
    def __init__(
        ...
        if self.apply_template:
            self.full_observation_spec[self.text_key] = NonTensor(
                shape=self.batch_size, example_data="a string", device=self.device
            )
        self.full_action_spec = Composite(
            {
                self.response_key: NonTensor(
                    shape=self.batch_size, example_data="a string", device=self.device
                )
            },
    ...
    def _step(self, tensordict):
        ...
        if self.apply_template:
            td_out[self.text_key] = history.apply_chat_template(
