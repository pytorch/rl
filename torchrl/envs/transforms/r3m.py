...
import omegaconf
import torch
from Cython.Includes.libc.stdio import tmpfile
...
def load_r3m(modelid, device, store_dir=None):
    if store_dir is None:
        store_dir = tmpfile.TemporaryDirectory()
    if modelid == "resnet50":
        foldername = "r3m_50"
    ...
    download_url(modelurl, store_dir, "model.pt")
    print("downloading config")
    download_url(configurl, store_dir, "config.yaml")
    print("done")
