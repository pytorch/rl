...
class DMControlEnv(GymLikeEnv):
    ...
    def _build_env(self, envname: str, taskname: str, _seed: Optional[int] = None, from_pixels: bool = False,
                   render_kwargs: Optional[dict] = None, **kwargs):
        if not _has_dmc:
    ...
    def _set_seed(self, _seed: Optional[int]) -> Optional[int]:
        self._env = self._build_env(self.envname, self.taskname, _seed=_seed, **self.constructor_kwargs)
        self.reset()
        return _seed
