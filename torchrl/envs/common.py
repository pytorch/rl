...
class Specs:
    ...
    def build_tensor_dict(
            self, next_observation=True, log_prob=False
    ):
...
class _EnvClass:
    ...
    def _assert_tensordict_shape(self, tensor_dict):
    ...
    def is_done_get_fn(self):
        return self._is_done.all()

    def is_done_set_fn(self, val):
        self._is_done = val

    is_done = property(is_done_get_fn, is_done_set_fn)

    def rand_step(self, tensor_dict: Optional[_TensorDict] = None):
        if tensor_dict is None:
    ...
    def _select_observation_keys(self, tensor_dict):
        for key in tensor_dict.keys():
            if key.rfind("observation") >= 0:
                yield key

    def return_current_tensordict(self):
...
class _EnvWrapper(_EnvClass):
    ...
    def _build_env(self, envname, taskname):
        raise NotImplementedError
...
class GymLikeEnv(_EnvWrapper):
    ...
    def _step(self, tensor_dict: _TensorDict):
    ...
    def _reset(self, tensor_dict: _TensorDict = None) -> _TensorDict:
    ...
    def _read_obs(self, observations):
