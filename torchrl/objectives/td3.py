...
class TD3Loss_deprecated(LossModule):
    """TD3 Loss module.
    ...
    def _call_qvalue(self, tensordict, tensordict_actor):

        # repeat tensordict_actor to match the qvalue size
        _actor_loss_td = (
            tensordict_actor[0]
            .select(*self.qvalue_network.in_keys)
            .expand(self.num_qvalue_nets, *tensordict_actor.batch_size[1:])
        )  # for actor loss
    ...
    def _call_actor(self, tensordict):
        act = tensordict.get(self.tensor_keys.action)
        action_shape = act.shape
        action_device = act.device
        ...
        tensordict_actor_grad, next_td_actor = self._vmap_actor_network00(
            tensordict_actor,
            self._cached_stack_actor_params,
        ).unbind(0)

        # add noise to target policy
        action = tensordict_actor_grad.get(self.tensor_keys.action)
        next_action = (next_td_actor.get(self.tensor_keys.action) + noise).clamp(
            self.min_action, self.max_action
        )
        tensordict_actor.set(
            self.tensor_keys.action,
            torch.stack([action, next_action], 0),
        )
        return tensordict_actor

    @dispatch
    def forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        tensordict_save = tensordict
        tensordict = tensordict.clone(False)
        tensordict_actor = self._call_actor(tensordict)
        (
            state_action_value_actor,
            next_state_action_value_qvalue,
            state_action_value_qvalue,
        ) = self._call_qvalue(tensordict, tensordict_actor)
        loss_actor = -(state_action_value_actor.min(0)[0]).mean()
