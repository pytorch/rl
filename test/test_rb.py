...
def test_prb(priority_key):
    ...
    td1 = TensorDict(
        source={'a': torch.randn(3, 1), priority_key: torch.rand(3, 1), '_idx': torch.arange(3).view(3, 1)},
        batch_size=[3])
    rb.extend(td1)
    s = rb.sample(2)
    assert s.batch_size == torch.Size([2, ])
    assert (td1[s.get("_idx").squeeze()].get("a") == s.get("a")).all()
    assert_allclose_td(td1[s.get("_idx").squeeze()].select("a"), s.select("a"))

    # test replacement
    td2 = TensorDict(
        source={'a': torch.randn(5, 1), priority_key: torch.rand(5, 1), '_idx': torch.arange(5).view(5, 1)},
        batch_size=[5])
    rb.extend(td2)
    s = rb.sample(5)
    ...
    # test strong update
    s.set_at_(priority_key, torch.ones(1, ) * 10000, 0)
    idx0 = s.get("_idx")[0]
    rb.update_priority(s)
    s = rb.sample(5)
    assert s.get("a").unique().numel() == 1
    assert td2[idx0].get(priority_key) == td2.get(priority_key).max()
    torch.testing.assert_allclose(td2[idx0].get("a").view(1), s.get("a").unique().view(1))
