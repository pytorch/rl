...
class TestDiscreteActionValueSelectionRegression:
    ...
    Each test calls the public loss API with fully controlled, hand-crafted
    inputs and asserts exact numeric output. If the action-value indexing
    logic is broken — wrong keepdim, wrong gather, or missing squeeze — the
    selected element changes, the loss value changes, and the assertion fails.
    """
    ...
    def test_dqn_categorical_selects_correct_action_value(self):
        ...
        """
        torch.manual_seed(0)
        net = nn.Linear(3, 3, bias=False)
        with torch.no_grad():
            net.weight.copy_(torch.eye(3))
