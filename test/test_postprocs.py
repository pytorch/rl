...
class TestMultiStep:
    ...
    def test_multistep(self, n, key, device, T=11):
        torch.manual_seed(0)
        ...
        assert (
            (next_obs == true_next_obs).all(-1) | terminated[:, (ms.n_steps) :]
        ).all()

        # test gamma computation
        torch.testing.assert_close(
            ms_tensordict.get("gamma"),
            ms.gamma ** ms_tensordict.get("steps_to_next_obs"),
        )
    ...
    def test_mutistep_cattrajs(
        self, batch_size, T, obs_dim, unsq_reward, last_done, device, n_steps
    ):
    ...
    @pytest.mark.parametrize("unsq_reward", [True, False])
    def test_unusual_done(self, unsq_reward):
        batch_size = [10, 3]
        T = 10
        obs_dim = [
            1,
        ...
        if unsq_reward:
            with pytest.raises(
                RuntimeError, match="tensordict shape must be compatible"
            ):
                _ = ms(td)
        else:
...
class TestDensifyReward:
    def test_densify_reward(self):
        ...
        assert new_tensordict is tensordict
        assert (
            new_tensordict["next", "reward"]
            == torch.cat(
                [
                    torch.full((4, 1), 3.0),
                    torch.full((4, 1), 7.0),
