...
class TestTQC(LossModuleTestBase):
    seed = 0
    ...
    @pytest.mark.skipif(not _has_functorch, reason="functorch not installed")
    @pytest.mark.parametrize("device", get_default_devices())
    @pytest.mark.parametrize("td_est", [ValueEstimators.TD0, None])
    @pytest.mark.parametrize("use_action_spec", [True, False])
    def test_tqc(
        self,
        device,
        ...
        assert all(
            (p.grad is None) or (p.grad == 0).all()
            for p in loss_fn.actor_params.values(True, True)
        )
        # check that losses are independent
        for k in loss.keys():
            if not k.startswith("loss"):
            ...
            if k == "loss_actor":
                ...
                assert loss_fn.log_alpha.grad.norm() > 0
            else:
                raise NotImplementedError(k)
            loss_fn.zero_grad()
        ...
        with pytest.warns(UserWarning, match="No target network updater has been"):
            ...
            for k in loss.keys():
                ...
                if k == "loss_actor":
                    assert all(
                        (p.grad is None) or (p.grad == 0).all()
                        for p in loss_fn.crtic_params.values(True, True)
                    )
                    assert not any(
                        (p.grad is None) or (p.grad == 0).all()
                        for p in loss_fn.actor_params.values(True, True)
    ...
    def test_tqc_batcher(
        ...
        # Check param update effect on targets
        target_actor = loss_fn.target_actor_params.clone().values(
            include_nested=True, leaves_only=True
        )
        target_qvalue = loss_fn.target_critic_params.clone().values(
            include_nested=True, leaves_only=True
        )
        for p in loss_fn.parameters():
            if p.requires_grad:
                p.data += torch.randn_like(p)
        target_actor2 = loss_fn.target_actor_params.clone().values(
            include_nested=True, leaves_only=True
        )
        target_qvalue2 = loss_fn.target_critic_params.clone().values(
            include_nested=True, leaves_only=True
        )
        if loss_fn.delay_actor:
            assert all((p1 == p2).all() for p1, p2 in zip(target_actor, target_actor2))
    ...
    def test_tqc_tensordict_keys(self, td_est):
        actor = self._create_mock_actor()
        value = self._create_mock_value()
        loss_fn = TQCLoss(
            actor,
