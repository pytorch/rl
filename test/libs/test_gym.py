...
class TestGym:
    ...
    def test_serial_env_indexing_returns_live_view(self):
        env = SerialEnv(4, functools.partial(GymEnv, PENDULUM_VERSIONED()))
        try:
            env.reset()
            sub_env = env[2:]
            try:
                assert isinstance(sub_env, SerialEnv)
                assert sub_env.num_workers == 2
                assert sub_env.batch_size == torch.Size([2])
                assert sub_env.reset().batch_size == torch.Size([2])
                assert sub_env.step(sub_env.rand_action()).batch_size == torch.Size(
                    [2]
                )
            finally:
                sub_env.close(raise_if_closed=False)

            singleton_env = env[0]
            try:
                assert isinstance(singleton_env, SerialEnv)
                assert singleton_env.num_workers == 1
                assert singleton_env.batch_size == torch.Size([1])
            finally:
                singleton_env.close(raise_if_closed=False)

            tensor_env = env[torch.tensor([1, 3])]
            try:
                assert isinstance(tensor_env, SerialEnv)
                assert tensor_env.num_workers == 2
                assert tensor_env.batch_size == torch.Size([2])
            finally:
                tensor_env.close(raise_if_closed=False)

            with pytest.raises(NotImplementedError, match="Boolean masks"):
                env[torch.ones(4, dtype=torch.bool)]
        finally:
            env.close(raise_if_closed=False)

    def test_parallel_env_indexing_returns_live_view(self, maybe_fork_ParallelEnv):
        env = maybe_fork_ParallelEnv(4, functools.partial(GymEnv, PENDULUM_VERSIONED()))
        try:
            env.reset()
            sub_env = env[2:]
            try:
                assert isinstance(sub_env, ParallelEnv)
                assert sub_env.num_workers == 2
                assert sub_env.batch_size == torch.Size([2])
                assert sub_env.reset().batch_size == torch.Size([2])
                assert sub_env.step(sub_env.rand_action()).batch_size == torch.Size(
            ...
            singleton_env = env[0]
            try:
                assert isinstance(singleton_env, ParallelEnv)
                assert singleton_env.num_workers == 1
                assert singleton_env.batch_size == torch.Size([1])
            finally:
                singleton_env.close(raise_if_closed=False)
            ...
            tensor_env = env[torch.tensor([1, 3])]
            try:
                assert isinstance(tensor_env, ParallelEnv)
                assert tensor_env.num_workers == 2
                assert tensor_env.batch_size == torch.Size([2])
            finally:
                tensor_env.close(raise_if_closed=False)
