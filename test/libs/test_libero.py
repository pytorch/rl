...
class TestLibero:
    ...
    def test_grouped_init_state_group_id_mode(self):
        env = LiberoEnv(
            "libero_spatial",
            task_id=0,
            init_state_mode="fixed",
            ...
            group_repeats=1,
            group_id_offset=100,
            group_id_mode="init_state",
            **_FAST,
        )
        try:
            group_ids = [env.reset()["group_id"].item() for _ in range(3)]
            assert group_ids == [103, 103, 103]
        finally:
            env.close(raise_if_closed=False)
    ...
    def test_render_gpu_device_id(self):
        env = LiberoEnv(
            "libero_spatial",
            task_id=0,
    ...
    def test_from_pixels(self):
        ...
        try:
            ...
            assert td["observation", "image"].shape == (3, 64, 64)
            assert td["observation", "image"].is_contiguous()
    ...
    def test_validation(self):
        with pytest.raises(ValueError, match="render_gpu_device_id"):
            LiberoEnv(
                ...
                env_kwargs={"render_gpu_device_id": 1},
                **_FAST,
            )
        with pytest.raises(ValueError, match="init_state_mode"):
            LiberoEnv("libero_spatial", task_id=0, init_state_mode="bad", **_FAST)
        with pytest.raises(ValueError, match="group_id_mode"):
            LiberoEnv("libero_spatial", task_id=0, group_id_mode="bad", **_FAST)
        with pytest.raises(ValueError, match="group_id_mode='init_state'"):
            LiberoEnv(
                "libero_spatial",
                task_id=0,
                group_repeats=1,
