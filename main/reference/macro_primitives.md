...
`URScriptPrimitiveTransform` maps `reach_pose` targets to
...
documented by the `CartesianSolver` protocol. A plain
...
The [`make_urscript_transform()`](generated/torchrl.envs.CubeBowlEnv.html#torchrl.envs.CubeBowlEnv.make_urscript_transform) preset honors
...
`TypeError` pointing at the `CartesianSolver` contract
