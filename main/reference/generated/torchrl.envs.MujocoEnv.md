...
*class*torchrl.envs.MujocoEnv(**args*, *num_workers: int = 1*, *parallel: bool | None = None*, ***kwargs*)[[source]](../../_modules/torchrl/envs/custom/mujoco/base.html#MujocoEnv)

Base TorchRL environment backed by a swappable MuJoCo physics engine.

Subclasses implement the task by overriding `_compute_reward()` and
`_compute_done()` (and optionally `_make_obs()`,
`_make_obs_spec()`, `_sample_initial_state()`,
`_prepare_ctrl()`, `_patch_xml()`).

The XML asset is resolved in this order: explicit `xml_path` kwarg >
class attribute `XML_PATH` > class attribute `XML_URL`.
...
`_patch_xml()`. If `False`, pass a local `xml_path` to
...
`qvel` at reset; defaults to `RESET_NOISE_SCALE`.
- **max_episode_steps** - per-env truncation horizon. The env emits
`terminated` (from `_compute_done()`), `truncated`
...
```
>>> from torchrl.envs import HumanoidEnv 
>>> env = HumanoidEnv(num_envs=4) 
>>> td = env.rollout(10)
```
...
geom_contacts(*geom_names: Sequence[str]*) → [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)[[source]](../../_modules/torchrl/envs/custom/mujoco/base.html#MujocoEnv.geom_contacts)
...
get_state() → [TensorDict](https://docs.pytorch.org/tensordict/stable/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict)[[source]](../../_modules/torchrl/envs/custom/mujoco/base.html#MujocoEnv.get_state)
...
render(***, *width: int | None = None*, *height: int | None = None*, *camera_id: int | None = None*) → [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)[[source]](../../_modules/torchrl/envs/custom/mujoco/base.html#MujocoEnv.render)
...
site_positions(*site_names: Sequence[str]*) → [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)[[source]](../../_modules/torchrl/envs/custom/mujoco/base.html#MujocoEnv.site_positions)
