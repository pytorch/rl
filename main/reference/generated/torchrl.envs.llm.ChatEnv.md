...
*class*torchrl.envs.llm.ChatEnv(**args*, *with_tokenizer: bool = False*, ***kwargs*)[[source]](../../_modules/torchrl/envs/llm/chat.html#ChatEnv)

A chat-based environment for LLMs, designed as a blank canvas for conversation and RL.

This environment is designed to work seamlessly with both `TransformersWrapper` and
`vLLMWrapper`. It provides the fundamental structure for managing conversation state
using the [`History`](torchrl.data.llm.History.html#torchrl.data.llm.History) format (or, alternatively, tokens or text), but is intentionally minimal to allow
...
> - **History mode**: Uses [`History`](torchrl.data.llm.History.html#torchrl.data.llm.History) objects for conversation management
...
> - **Reward computation**: [`KLRewardTransform`](torchrl.envs.llm.transforms.KLRewardTransform.html#torchrl.envs.llm.transforms.KLRewardTransform) for KL divergence rewards
> - **Tool execution**: [`PythonInterpreter`](torchrl.envs.llm.transforms.PythonInterpreter.html#torchrl.envs.llm.transforms.PythonInterpreter) for Python code execution
> - **Data loading**: [`DataLoadingPrimer`](torchrl.envs.llm.transforms.DataLoadingPrimer.html#torchrl.envs.llm.transforms.DataLoadingPrimer) for loading prompts from datasets
> - **Thinking prompts**: [`AddThinkingPrompt`](torchrl.envs.llm.transforms.AddThinkingPrompt.html#torchrl.envs.llm.transforms.AddThinkingPrompt) for chain-of-thought reasoning
> - **Token maintenance**: [`IncrementalTokenizer`](torchrl.envs.llm.transforms.IncrementalTokenizer.html#torchrl.envs.llm.transforms.IncrementalTokenizer) for token-first inference

Keyword Arguments:

- **input_mode** (*Literal**[**"history"**,**"text"**,**"tokens"**]*) - The mode of input to the environment.
Defaults to "history".
- **batch_size** ([*torch.Size*](https://docs.pytorch.org/docs/stable/size.html#torch.Size)) - Expected batch size of the input. Defaults to (1,) (null batch sizes such as ()
...
Defaults to None.
- **template_kwargs** (*dict**[**str**,**any**]**,**optional*) - Keyword arguments passed to [`apply_chat_template()`](torchrl.data.llm.History.html#torchrl.data.llm.History.apply_chat_template).
Defaults to None.
...
Makes a step in the environment. A tensordict or equivalent with the LLM's response must be passed.
The response key is defined as a class attribute response_key.

See also

To see examples of a ChatEnv in action, see [`DatasetChatEnv`](torchrl.envs.llm.DatasetChatEnv.html#torchrl.envs.llm.DatasetChatEnv),
[`GSM8KEnv`](torchrl.envs.llm.GSM8KEnv.html#torchrl.envs.llm.GSM8KEnv) and [`IFEvalEnv`](torchrl.envs.llm.IFEvalEnv.html#torchrl.envs.llm.IFEvalEnv).
...
[DatasetChatEnv](torchrl.envs.llm.DatasetChatEnv.html#torchrl.envs.llm.DatasetChatEnv)
...
*classmethod*with_tokenizer(*tokenizer: transformers.AutoTokenizer*, ***kwargs*) → [TransformedEnv](torchrl.envs.transforms.TransformedEnv.html#torchrl.envs.transforms.TransformedEnv)[[source]](../../_modules/torchrl/envs/llm/chat.html#ChatEnv.with_tokenizer)
...
A ChatEnv wrapped with IncrementalTokenizer.
