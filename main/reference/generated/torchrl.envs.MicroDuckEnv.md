...
`scene_walk.xml`. Defaults to the `ROOT_ENV_VAR`
...
`MICRODUCK_RL_COMMIT` of `microduck_rl` into `root` when
...
```
>>> env = MicroDuckEnv(download=True, num_envs=1024, device="cuda", compile_step=True) 
>>> env = MicroDuckEnv(download=True, backend="mjx", num_envs=1024, device="cuda") 
...
Pollen Robotics, MicroDuck ([pollen-robotics/microduck](https://github.com/pollen-robotics/microduck))
...
foot_contacts() → [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)[[source]](../../_modules/torchrl/envs/custom/mujoco/microduck.html#MicroDuckEnv.foot_contacts)

Return a `(num_envs, 2)` boolean tensor of left/right foot contact.

foot_heights() → [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)[[source]](../../_modules/torchrl/envs/custom/mujoco/microduck.html#MicroDuckEnv.foot_heights)
...
*classmethod*resolve_scene(*microduck_root: str | Path | None = None*, ***, *root: str | Path | None = None*, *download: bool | str = False*) → Path[[source]](../../_modules/torchrl/envs/custom/mujoco/microduck.html#MicroDuckEnv.resolve_scene)
...
`ROOT_ENV_VAR` environment variable, an installed
`mjlab_microduck` package and a checkout of
`MICRODUCK_RL_COMMIT` under `root` are tried in that
order.
