...
decode(***, *device: Any = None*, *dtype: Any = None*) → [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)[[source]](../../_modules/torchrl/data/video.html#VideoClipRef.decode)
...
the decoded frames (`uint8` by default).
...
*classmethod*from_file(*path: str | PathLike*, ***, *stream: int | None = None*, *num_frames: int | None = None*, *frame_index: [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor) | None = None*, *num_bins: int | None = None*, *frames_per_bin: int | None = None*, *auto_decode: bool = False*, *device: Any = None*, *dtype: Any = None*) → VideoClipRef[[source]](../../_modules/torchrl/data/video.html#VideoClipRef.from_file)
...
- **frame_index** ([*torch.Tensor*](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)*,**optional*) - explicit frame positions. If
...
non-overlapping temporal bins (see `rebin()`). Defaults to
`None` (every frame).
...
- **auto_decode** (*bool**,**optional*) - see `VideoClipRef`. Defaults to
`False`.
- **device** ([*torch.device*](https://docs.pytorch.org/docs/stable/tensor_attributes.html#torch.device)*or**str**,**optional*) - default output device. A CUDA
device uses GPU (NVDEC) decoding when supported, else decodes on CPU
and moves the frames. Defaults to `None`.
- **dtype** ([*torch.dtype*](https://docs.pytorch.org/docs/stable/tensor_attributes.html#torch.dtype)*,**optional*) - default decode dtype. Defaults to
`None` (`uint8`).

Returns:

a reference whose batch size is the number of frames (or
...
*classmethod*from_files(*paths: Sequence[str | PathLike]*, ***, *num_frames_per_file: int | Sequence[int] | None = None*, *num_bins: int | None = None*, *frames_per_bin: int | None = None*, *stream: int | None = None*, *auto_decode: bool = False*, *device: Any = None*, *dtype: Any = None*) → VideoClipRef[[source]](../../_modules/torchrl/data/video.html#VideoClipRef.from_files)
...
`rebin()` and decoding work across file boundaries (a slice that
...
- **auto_decode** (*bool**,**optional*) - see `VideoClipRef`.
- **device** ([*torch.device*](https://docs.pytorch.org/docs/stable/tensor_attributes.html#torch.device)*or**str**,**optional*) - default output device.
- **dtype** ([*torch.dtype*](https://docs.pytorch.org/docs/stable/tensor_attributes.html#torch.dtype)*,**optional*) - default decode dtype.

Returns:

a reference over the concatenated frames of all files.
...
```
>>> ref = VideoClipRef.from_files(["ep0.mp4", "ep1.mp4"]) 
>>> ref.rebin(30, frames_per_bin=4).decode().shape 
...
*classmethod*from_timestamps(*path: str | PathLike*, *timestamps: [Tensor](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)*, ***, *stream: int | None = None*, *fps: float | None = None*, *auto_decode: bool = False*, *device: Any = None*, *dtype: Any = None*) → VideoClipRef[[source]](../../_modules/torchrl/data/video.html#VideoClipRef.from_timestamps)
...
- **path** (*str**or**PathLike*) - the video path / URI / file id.
- **timestamps** ([*torch.Tensor*](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)) - timestamps in seconds.

Keyword Arguments:

- **stream** (*int**,**optional*) - stream index. Defaults to `None`.
- **fps** ([*float*](torchrl.data.llm.TopKRewardSelector.html#torchrl.data.llm.TopKRewardSelector.float)*,**optional*) - frames-per-second to use for the conversion.
Defaults to `None` (read from metadata).
- **auto_decode** (*bool**,**optional*) - see `VideoClipRef`.
- **device** ([*torch.device*](https://docs.pytorch.org/docs/stable/tensor_attributes.html#torch.device)*or**str**,**optional*) - default decode device.
- **dtype** ([*torch.dtype*](https://docs.pytorch.org/docs/stable/tensor_attributes.html#torch.dtype)*,**optional*) - default decode dtype.

Returns:

a reference addressing the nearest frames to `timestamps`.
...
rebin(*num_bins: int*, *frames_per_bin: int | None = None*) → VideoClipRef[[source]](../../_modules/torchrl/data/video.html#VideoClipRef.rebin)
...
(`rebin(num_bins)`) and apply [`CatFrames`](torchrl.envs.transforms.CatFrames.html#torchrl.envs.transforms.CatFrames)
...
a new (lazy) reference over the binned frames.
...
```
>>> ref = VideoClipRef(path) # 100 frames 
