...
*class*torchrl.envs.llm.LLMEnv(**args*, ***kwargs*)[[source]](../../_modules/torchrl/envs/llm/envs.html#LLMEnv)
...
The default arguments of the LLMEnv class are set to make it easy to run this environment with
the vllm backend (`vLLMWrapper`).
...
- **device** ([*torch.device*](https://docs.pytorch.org/docs/stable/tensor_attributes.html#torch.device)*|**None**,**optional*) - The device on which the environment should run. Defaults to `None`.
...
`attention_key`. Defaults to `True`.
...
- **assign_done** (*bool**,**optional*) -

If `True`, a zero-valued done and terminated state of shape equal to the
action shape is written during calls to step(). Defaults to `False`.

Note

Regardless of the value assigned to assign_done, a done state will be written at the root
as it is a requirement for all TorchRL environments.
- **batch_size** (*int**or*[*torch.Size*](https://docs.pytorch.org/docs/stable/size.html#torch.Size)*,**optional*) -
...
When using a `DataLoadingPrimer` transform, the batch-size of the env
...
`DataLoadingPrimer` for examples.

from_dataloader()[[source]](../../_modules/torchrl/envs/llm/envs.html#LLMEnv.from_dataloader)
...
*classmethod*from_dataloader(*dataloader: DataLoader*, ***, *tokenizer: transformers.PretrainedTokenizerBase | None = None*, *token_key: NestedKey | None = None*, *str_key: NestedKey | None = None*, *attention_key: NestedKey | None = None*, *action_key: NestedKey | None = None*, *reward_key: NestedKey = 'reward'*, *from_text: bool = True*, *device: [torch.device](https://docs.pytorch.org/docs/stable/tensor_attributes.html#torch.device) | None = None*, *vocab_size: int | None = None*, *batch_size: int | [torch.Size](https://docs.pytorch.org/docs/stable/size.html#torch.Size) | None = None*, *has_attention: bool = True*, *assign_reward: bool = False*, *assign_done: bool = False*, *primers: [Composite](torchrl.data.Composite.html#torchrl.data.Composite) | None = None*, *example_data: Any = None*, *stack_method: Callable[[Any], Any] | Literal['as_nested_tensor', 'as_padded_tensor'] = None*, *repeats: int | None = None*, *group_repeats: bool = True*, *eos_token_id: int | None = None*) → LLMEnv[[source]](../../_modules/torchrl/envs/llm/envs.html#LLMEnv.from_dataloader)
...
- **tokenizer** (*transformers.PretrainedTokenizerBase**or**str**,**optional*) -

the tokenizer to use. If `None`,
"bert-base-uncased" will be used by default. If a string is provided, it should be the name of a
pre-trained tokenizer.

Note

Using the tokenizer will append a `Tokenizer` transform to the environment.
...
if `True`, a zero-valued done and terminated state of shape equal to to the
action shape is written during calls to step(). Defaults to `False`.
...
When using a `DataLoadingPrimer` transform, the batch-size of the env
and the transform should match.
- **primers** ([*Composite*](torchrl.data.Composite.html#torchrl.data.Composite)*|**None**,**optional*) - The primers to use for each key in the dataloader.
