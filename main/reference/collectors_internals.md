...
A single iteration of [`Collector.rollout()`](generated/torchrl.collectors.Collector.html#torchrl.collectors.Collector.rollout) corresponds to one
...
Implementation: [`Collector.rollout()`](generated/torchrl.collectors.Collector.html#torchrl.collectors.Collector.rollout) in
`torchrl/collectors/_single.py`.

## The carrier

The **carrier** is the [`TensorDictBase`](https://docs.pytorch.org/tensordict/stable/reference/generated/tensordict.TensorDictBase.html#tensordict.TensorDictBase) instance stored as
...
[`Collector.post_collect_hook`](generated/torchrl.collectors.Collector.html#torchrl.collectors.Collector.post_collect_hook) or read the data yielded by
...
| Non-CUDA, CUDA available | `_cuda_sync_if_initialized` (safe to call after a GPU-to-host transfer; no-op in fork subprocesses where CUDA was not initialised) |
| Non-CUDA, MPS available | `torch.mps.synchronize` |
| Non-CUDA, NPU available | `torch.npu.synchronize` |
...
- `Collector._traj_pool()` returns a process-local
`torchrl.collectors.utils._TrajectoryPool` that hands out fresh IDs.
...
- `Collector._update_traj_ids()` runs after each env step. It reads
...
Called once at the top of `rollout()`, before the per-timestep loop
...
consumer. Receives the [`TensorDictBase`](https://docs.pytorch.org/tensordict/stable/reference/generated/tensordict.TensorDictBase.html#tensordict.TensorDictBase) that will be
...
| --- | --- |
| Per-step rollout loop | [`Collector.rollout()`](generated/torchrl.collectors.Collector.html#torchrl.collectors.Collector.rollout) in `torchrl/collectors/_single.py` |
| Carrier initialization | `Collector._make_carrier()` |
| Device setup and policy cast flag | `Collector._setup_devices()` |
| Environment device application and env cast flag | `Collector._apply_env_device()` |
| Sync function dispatch | `Collector._get_sync_fn()` |
| Trajectory ID update | `Collector._update_traj_ids()` |
| Trajectory ID pool | `torchrl.collectors.utils._TrajectoryPool` |
| Hooks | `pre_collect_hook` / `post_collect_hook` constructor arguments |
