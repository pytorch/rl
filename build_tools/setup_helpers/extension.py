...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        ...
        try:
            output = check_output(["cmake", str(_ROOT_DIR)] + cmake_args, cwd=self.build_temp, stderr=STDOUT)
        except CalledProcessError as exc:
            print(exc.output)

        try:
            output = check_output(["cmake", "--build", "."] + build_args, cwd=self.build_temp, stderr=STDOUT)
        except CalledProcessError as exc:
            print(exc.output)
