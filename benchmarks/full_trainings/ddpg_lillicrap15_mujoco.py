...
def make_ddpg_agent(env_name, device):
    """Make DDPG agent."""
...
def make_logger(backend="csv"):
    exp_name = generate_exp_name("DDPG", f"Mujoco_lillicrap15_{env_name}")
    logger = get_logger(backend, logger_name="ddpg", experiment_name=exp_name)
    return logger
...
if __name__ == "__main__":
    ...
    # Make the components
    model, model_explore = make_ddpg_agent(env_name, device)
